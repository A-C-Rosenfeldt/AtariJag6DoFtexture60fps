...
import { Camera_in_stSpace, Mapper } from './infinite_plane_mapper.js';
...
class Polygon_in_cameraSpace {
    ...
    project(vertices) {
        ...
        if (on_screen.length == 0) {
            ...
            if (null_egal == 0)
                return; // polygon completely outside of viewing frustm
        ...
        if (on_screen.length > 0) {
            ...
            this.rasterize_onscreen(pixel_coords, payload);
            return true;
        }
    ...
    edge_crossing_two_borders(vertex, pattern4, on_screen) {
    ...
    rasterize_onscreen(vertex, Payload) {
        ...
        do {
            for (let k = 0; k < 2; k++) {
                if (y == vertex[active_vertices[k][1]][1]) {
                    ...
                    */
                    ...
                    if (d[1] != 0)
                        slope_accu_c[k][0] = d[0] / d[1]; // we only care for edges with actual height on screen. And exact vertical will not glitch too badly
            ...
            let ny = Math.min(...active_vertices.map(a => vertex[a[1]][1])), xo, anchor = false;
            for (; y < ny; y++) { // floating point trick to get from NDC to pixel cooridinates. 256x256px tech demo. Square pixels. Since I don't round, any factor is possible. Easy one is 256  * 5/4-> 320  .. 256 * 3/4 = 192				
                ...
                let x = integer[0];
                if (x < integer[1]) { // dragged out of the for loop. Not an actual additional jump in JRISC
                    ...
                    if (anchor) { // I run out of registers, but actually would like to keep more temporary values. MOVTA ? So her is just a quick hack for half of all slopes
                        ...
                        switch (dx) {
                            case -1:
                                payload_w.forEach((p, i) => { fragment[i] += p[2] - p[1]; });
                                break;
                            case +1:
                                payload_w.forEach((p, i) => { fragment[i] += p[2] + p[1]; });
                                break;
                            case 0:
                                payload_w.forEach((p, i) => { fragment[i] += p[2]; });
                                break; // this will speed up all edges clipped at the screen borders
                    ...
                    if (fralment.length > 2) { // Z=0 U=1 V=2 coordinates need perspective correction. Gouraud on this blitter is Z=0 G=1
                        ...
                        if (span_within > 0 && fralment[0] > 0.001) { // z>0 safety first
                            ...
                            if (span_within == 0 || right_side[0] <= 0.001)
                                continue;
                            ...
                            for (let uv = 0; uv < 2; uv++) {
                                ...
                                if (span_within)
                                    blitter_slope[1 + uv] = d;
                                else
                                    blitter_slope[1 + uv] = d / span_within; // linear interpolation. Quake bloats the code for small values. I have some JRISC ideas in the project: scan for first bit. shift one more. Zero flag? then apply shift to argument. Else: div
                            }
                        }
                    }
                    xo = x;
                ...
                for (let k = 0; k < 2; k++)
                    slope_accu_c[k][1] += slope_accu_c[k][0];
            }
