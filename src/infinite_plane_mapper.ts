...
export class Camera_in_stSpace{
	...
	*/
	...
	pg=[[1,0],[0,1]]  // can load uv mapping into this: z needs to be generated
	...
	generate_payload_m(C:number[]):CV{
	...
	infinte_checkerBoard(C:number[],V:number[]):number[]{
 	...
 	infinte_checkerBoard_m(C:number[]):CV{
		let cv=new CV
	...
	transform_into_texture_space(v:number[]):number[]{
		let v3=new Vec3([v])
		let coords=[0,0,0]
