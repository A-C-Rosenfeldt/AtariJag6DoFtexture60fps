...
export class Gradient {
	...
	private _slope = new Array<number>(2);
	public get slope() {
		return this._slope;
	}
	public set slope(value) {
		if (value[1]<0){
...
export class Polygon_in_cameraSpace {
	...
	private edge_crossing_two_borders(vertex: Vertex_in_cameraSpace[], pattern4: number,veto_face_vertex:number[][]): Item[] {
		...
		*/
		...
		const on_screen = new Array<Item>(3)
		{
			for (let j = 0; j < 2; j++) {
	...
	private streamIn_newVertex(active_vertices: number[],other_verticex:number, Bresenham: Gradient, ind: Cyclic_Indexer, vertex: Item[], count_to_one: boolean): { x_at_y_int: number, overshot: boolean } {
		...
		for (let eat_edges = 0; eat_edges < 10 /* safety */; eat_edges++) {
			...
			if (edge instanceof Edge_w_slope) {
				...
				if (gradient.v[0]<0) gradient=gradient.scalarProduct(-1) // for akku
	...
	private clipped_edge_to_Bresenham(os:Vertex_OnScreen ,ob:Onthe_border, edge: Edge_w_slope, Bresenham: Gradient) {
	...
	*/
	...
	private edge_fromVertex_toBorder(vs: Vertex_in_cameraSpace[], l: number, reverse=false): Item[] {
		...
		*/
...

		{
			...
			var pixel_ordinate = vs[0].onScreen.position[1 ^ (border & 1)] ;console.log("straight",border,pixel_ordinate)
			const lll=0
		}
